...
#pragma region  Input-Ouput System
...
class polar {
    public:
        ...
        polar operator*(int x){
            return polar(r*x, arg); }
        polar operator*(long x){
            return polar(r*x, arg); }
        polar operator*(double x){
            return polar(r*x, arg); }
        polar operator*(float x){
            return polar(r*x, arg); }
...
polar pole(complex z){
    return polar(z.abs(), z.arg()); }
...
class Mathf{
    public:
        vector<int> RootConvergents(long n){
            vector<int> c;
            double x = pow(n,0.5);
            if(((x - floor(x)) == 0)){
                return vector<int>{0};
...
int main(){
    ...
    cout<<"sqroot convergents of 511 are : ";
    vector<int> p = MathX.RootConvergents(511);
